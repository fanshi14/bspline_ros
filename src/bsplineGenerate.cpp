...
void bsplineGenerate::pathGridPointsCallback(const geometry_msgs::PolygonStampedConstPtr& msg)
{
  ...
  if (isTsNone)
    ...
    if (m_n_control_pts < 2*m_deg+2)
      m_spline_ptr = new ts::BSpline((m_n_control_pts-1)/2-1, 3, m_n_control_pts, TS_CLAMPED);
    else
      m_spline_ptr = new ts::BSpline(m_deg, 3, m_n_control_pts, TS_CLAMPED);
  }
  ...
  m_tn = msg->polygon.points[2*m_n_control_pts-2].x;
